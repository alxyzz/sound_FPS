...
public class ActionManager : NetworkBehaviour
{
  ...
  private Transform playerCamera;
    ...
    private float currWepDelay;

    private void GetKeyboardInput()
    {
        currWepDelay -= Time.deltaTime;
                ...
                if (isLocalPlayer)
                {
                    ...
                    switch (currWep.ShootingType)
                    {
                        case WeaponData.WeaponFireType.Pistol: 
                            if (period < 0.25f) 
                        {
                            currWepDelay = currWep.FireDelay;
    ...
    private void ChangeWeapon(WeaponData w)
    {
        ...
        switch (currWep.ShootingType)
        {
            case WeaponData.WeaponFireType.Pistol:
                ...
                break;
            case WeaponData.WeaponFireType.SMG:
                maxPeriod = 4; period = 0;
                break;
            case WeaponData.WeaponFireType.Sniper:
                maxPeriod = 6; period = 0;
                break;
        }
    }
